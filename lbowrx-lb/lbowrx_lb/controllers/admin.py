...
from lbowrx_lb.controllers.session import SessionStorage
from lbowrx_lb.users import UserList
...
class Authentication(object):
    def getUser(self, request):
        if "lbowrx-session" not in request.cookies:
            return None
        session_id = request.cookies["lbowrx-session"].value
...
class AuthorizationMixin(object):
    ...
    def handle_request(self):
        if self.isAuthorized():
            ...
            cookie = SimpleCookie()
            cookie["lbowrx-session"] = ""
            cookie["lbowrx-session"]["expires"] = "Thu, 01 Jan 1970 00:00:00 GMT"
