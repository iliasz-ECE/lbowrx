/*
	...
	Copyright (c) 2024 by Ilias Zervas <iliasz-ece@mail.com>
...
var decided_sdrid = null;
var decided_profileid = null;
...
function on_ws_closed() {
    ...
    if (!new_ws){
        ...
        setTimeout(prepare_to_reconnect, reconnect_timeout);
    }
    else{
        //No reconnection
    }
...
var decided_server = null;

function open_websocket() {
    // Decided
    if(decision){
        ws_url = "ws://" + decided_server + "/ws/"
        ...
        divlog("Currently no available sdr for this profile", true);
    }
}
...
async function report_failed_sdr() {
    failed_sdrs = getCookie("failed_sdrs");
    if (failed_sdrs) {cookie_value = failed_sdrs + ',' + decided_sdrid;}
    else {cookie_value = decided_sdrid;}
    setCookie("failed_sdrs", cookie_value);
    $("#openwebrx-panel-receiver").demodulatorPanel().stopDemodulator();
    await sdr_profile_changed();
    // set again, because it is deleted after new ws.
    setCookie("failed_sdrs", cookie_value);
}

function prepare_to_reconnect() {
    if ((ws.readyState >= 2) && !new_ws) open_websocket();
}
...
async function wait_ws_to_close(){
    if(ws.readyState < 3){
        await sleep(wait_ws_to_close);
    }
}

async function sdr_profile_changed() {
    var selected_profile = $('#openwebrx-sdr-profiles-listbox').val();
    new_ws = true;
    if(ws){
        ws.close();
        await wait_ws_to_close();
        
        waterfall_clear();
    
        await $.get("http://" + location.host+ "/compiled/websocket-vars.js?profileid=" + selected_profile);
