...
class SdrDeviceDescription(object):
    ...
    def getDeviceInputs(self) -> List[Input]:
        ...
        return [TextInput("name", "Device name", validator=RequiredValidator())] + [NumberInput("priority", "Sdr priority", validator=RangeValidator(0,1000), infotext="Note: Sdr priority for load balancing. <br /> Note: default is 100, range: 0-1000, 0 means disabled.")] + [i for i in self.getInputs() if i.id in keys]
