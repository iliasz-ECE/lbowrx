...
from owrx.controllers.state import StateController
...
from owrx.form.input import TextInput, NumberInput, DropdownInput, Option
...
class SdrDeviceController(SdrFormControllerWithModal):
    ...
    def deleteDevice(self):
        ...
        StateController.update_state_tag()
...
class NewSdrDeviceController(SettingsFormController):
    ...
    def get_breadcrumb(self) -> Breadcrumb:
        return SdrDeviceBreadcrumb().append(BreadcrumbItem("New device", "settings/newsdr"))
    ...
    def getSections(self):
        return [
            Section(
                ...
                NumberInput("priority", "Sdr priority", validator=RangeValidator(0,1000), 
                    infotext="Note: Sdr priority for load balancing. <br /> "
                    + "Note: default is 100, range: 0-1000, 0 means disabled."
                ) 
            )
        ]
